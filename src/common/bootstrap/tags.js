...
/**
 * 获取热门推荐数据
 * {% hotrecommend data = "data"%}
 * topic:标签名称
 * data:接受返回数据的变量名称，例: data = "data"
 * page: 设置查询开始页面，从1开始，默认为0，例：page = "2"
 * limit: 设置查询结果的条数，例: limit="10",limit="3,10"
 * cid: 栏目id ,单个栏目 cid="1",多个栏目 cid = "1,2,3,4" , 不写调取全部栏目   固定为游学130和学校
 * {{name|get_url(id)}}文章链接
 * type: 标签类型,hot-按照浏览量从高到底,level-安装优先级从高到低排序,默认安装更新时间排序   固定为level
 * //{% topic data = "data",limit= "5",cid=category.id,type="hot"%}
 ...
 * pid:父分类
 */
 global.hotrecommend = function(){
    this.tags = ['hotrecommend'];
    this.parse = function (parser, nodes, lexer) {
        let tok = parser.nextToken();
        let args = parser.parseSignature(null, true);
        parser.advanceAfterBlockEnd(tok.value);
